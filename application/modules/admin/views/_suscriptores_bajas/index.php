<div class="titulo">Bajas de Suscriptores
    <div class="opciones_menu">
        <a href="<?= base_url() ?>admin/suscriptores_bajas/crear"><img class="ico_agregar" src="<?= base_url(); ?>css/general/img/t.gif" title="Dar de baja a suscriptor"></a>
    </div>
</div>
<div class="contenido">
    <div class="contenedortablabuscar">
        <form action="<?= base_url() ?>admin/suscriptores_bajas" id="form_buscar" method="post" >
            <table>
                <tbody>
                    <tr>
                        <td>Buscar:</td>
                        <td>
                            <input type="text" name="w_buscar" id="w_buscar" value="<?= $w_buscar ?>">
                            <input name="desde" id="desde" type="hidden" value="0" />
                        </td>
                        <td><img src="<?= base_url(); ?>css/general/img/t.gif" class="buscar" title='Buscar emails'></td>
                    </tr>
                </tbody>
            </table>
        </form>
    </div>

    <div class="contenedortabla">
        <table>
            <tr>
                <th width="2%">Id</th>
                <th width="43%">Nombre</th>
                <th width="40%">Correo</th>
                <th width="15%">Opciones</th>
            </tr>
            <?= $html ?>
            <tfoot>
                <tr>
                    <td colspan="4">
                        <?= $paginado ?>
                    </td>
                </tr>
            </tfoot>
        </table>
    </div>
</div>

<script src="<?= base_url() ?>js/admin/helper.js"></script>
<script src="<?= base_url() ?>js/admin/suscriptores_bajas.js"></script>